...
#         logging.info('RestGroupsListColtrolHandler:: commandName '+ str(commandName))
#         logging.info('RestGroupsListColtrolHandler:: curentParameter '+ str(curentParameter))
# 
#         link=self.get_argument('link', '')
#         logging.info('RestGroupsListColtrolHandler:: link = '+ str(link))
        
         
        self.get_current_user()
        self.curentAuthor = self.current_user
 
        logging.info( 'RestGroupColtrolHandler:: get self.curentAuthor = ' + str(self.curentAuthor))
...
#             groups += yield executor.submit(artHelper.getListGroups, config.options.tpl_categofy_id)
#             id = groupId 
            logging.info( 'RestGroupColtrolHandler:: groupId = ' + str(groupId))
            foundValue = None
            for item in souList:
            ...
            logging.info( 'RestGroupColtrolHandler:: foundValue = ' + str(foundValue))
            ...
            logging.info( ' RestGroupColtrolHandler get  Exception as et = ' + str(e))
            error = Error ('500', 'что - то пошло не так :-( ')
            self.write(json.dumps(error))
