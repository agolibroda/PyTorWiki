...
class AuthorsList(BaseHandler):
    """
    Показать список Авторов - просто список
    Это ответ на запрос 
    r"/authors"
    
    показываем только ФИО + картинку. 
    (возможно, список публикаций????) потом.
    ...
    def get(self):
        ...
        try:
            ...
            authorsControl = Author()
    
            tplControl = TemplateParams()
            tplControl.make(spectatorAuthor)
            tplControl.authorsList = yield executor.submit( authorsControl.list )

            tplControl.page_name='Список Авторов '
            tplControl.link='profile'
            tplControl.error=None
            ...
            #
            logging.info(" AuthorsList get  tplControl = " + str(tplControl))             
            self.render("authorsList.html", parameters=tplControl)
